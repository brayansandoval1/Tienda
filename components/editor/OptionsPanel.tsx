      ...
      {!product.options?.length ? <p className="text-sm text-gray-500">Este producto no tiene opciones configuradas.</p> : product.options.map((option) => (
        <section key={option.id}>
          ...
          {(option.displayType ?? option.type) === 'thumbnails' ? (
            ...
            <select value={selections[option.id] ?? ''} onChange={(event) => choose(option, option.values.find((value) => value.id === event.target.value)!)} className="w-full rounded-xl border border-gray-200 bg-white p-2.5 text-sm">
