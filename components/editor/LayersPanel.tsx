...
import { GripVertical, Image as ImageIcon, Shapes, Type } from 'lucide-react';
  ...
  return (
    <aside className="flex min-h-0 w-full max-w-[320px] flex-col bg-white" aria-label="Capas del diseño">
      <div className="border-b border-slate-200 px-5 py-4">
        ...
        <p className="mt-0.5 text-xs text-slate-500">Arrastra para cambiar el orden</p>
      </div>
            ...
            return (
              <div
                ...
                className={`relative flex w-full items-center gap-2 rounded-xl border p-2 text-left transition ${
                  selected ? 'border-slate-900 bg-slate-100 shadow-sm' : 'border-transparent hover:border-slate-200 hover:bg-slate-50'
                } ${draggedObject === object ? 'opacity-45' : ''}`}
              >
                ...
                <button type="button" onClick={() => selectObject(object)} className="flex min-w-0 flex-1 items-center gap-2 text-left">
                  ...
                  <span className="min-w-0">
                    <span className="block truncate text-sm font-semibold text-slate-800">{getLayerTitle(object)}</span>
                    <span className="block truncate text-[11px] text-slate-500">{getLayerContent(object)}</span>
                  </span>
                </button>
              </div>
            );
